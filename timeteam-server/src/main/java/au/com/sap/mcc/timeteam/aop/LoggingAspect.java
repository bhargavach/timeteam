package au.com.sap.mcc.timeteam.aop;
...
public class LoggingAspect
{
	 ...
	 */
	@Around("execution(* au.com.sap.mcc.timeteam.*.*(..))")
	public Object traceMethodInvocation(ProceedingJoinPoint joinPoint) throws Throwable
	{
		...
		logger.error("entering: {}", joinPoint.getSignature().toLongString());
		
		// proceed with intercepted call
		retVal = joinPoint.proceed();

		logger.error("exiting {}", joinPoint.getSignature().toLongString());
	 ...
	 */
	@AfterThrowing(pointcut="execution(* au.com.sap.mcc.timeteam.*.*(..))", throwing="ex")
